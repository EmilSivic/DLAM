...
class Config:
    ...
    # Optimization
    BATCH_SIZE: int = 8          # increase if VRAM allows (16/32/64); then set GRAD_ACCUM_STEPS=1
    EPOCHS: int = 8
    BASE_LR: float = 5e-4
    ...
    # Throughput boosters
    GRAD_ACCUM_STEPS: int = 2    # set to 1 if you can raise BATCH_SIZE
    USE_AMP: bool = True         # mixed precision
    NUM_WORKERS: int = 2         # try 2-4
    PIN_MEMORY: bool = True
    PREFETCH_FACTOR: int = 2
    PRETOKENIZE: bool = False    # True = tokenize once in __init__ (uses more RAM)
    COMPILE: bool = False        # True = torch.compile(model) if PyTorch >= 2.0

    # Generation/logging
    MAX_NEW_TOKENS: int = 256    # steps can be long
    LOG_EXAMPLES: int = 3
    LOG_EVERY_N_STEPS: int = 200 # mid-epoch heartbeat
    DEVICE: str = "cuda" if torch.cuda.is_available() else "cpu"

    # Epoch-level text-metric evaluation (generation-based)
    METRIC_EVAL_SAMPLES: int = 128      # -1 = all val rows; keep modest for speed
    METRIC_EVAL_SEED: int = 123

    # Optional caps for smoke tests (0 = all after subsetting)
...
class RecipeDatasetSP(Dataset):
    """
    Dataset that tokenizes with SentencePiece.
    If PRETOKENIZE=True, we precompute and store token IDs (faster per-step; more RAM).
    """
    def __init__(self, df: pd.DataFrame, sp: spm.SentencePieceProcessor):
        ...
        self.pre_tgt = None
        if cfg.PRETOKENIZE:
            self.pre_src, self.pre_tgt = [], []
            for i in range(len(self.df)):
                s, t = self._encode_pair(self.df.iloc[i]["input"], self.df.iloc[i]["target"])
                self.pre_src.append(torch.tensor(s, dtype=torch.long))
                self.pre_tgt.append(torch.tensor(t, dtype=torch.long))
    ...
    def _encode_pair(self, src_text: str, tgt_text: str):
        ...
        if len(src_ids) > cfg.MAX_SRC_LEN:
            src_ids = src_ids[:cfg.MAX_SRC_LEN]; src_ids[-1] = self.eos_id
        if len(tgt_ids) > cfg.MAX_TGT_LEN:
            tgt_ids = tgt_ids[:cfg.MAX_TGT_LEN]; tgt_ids[-1] = self.eos_id
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, str, str]:
        if cfg.PRETOKENIZE:
            return self.pre_src[idx], self.pre_tgt[idx], str(self.df.iloc[idx]["input"]), str(self.df.iloc[idx]["target"])
        src_ids, tgt_ids = self._encode_pair(self.df.iloc[idx]["input"], self.df.iloc[idx]["target"])
        return torch.tensor(src_ids, dtype=torch.long), torch.tensor(tgt_ids, dtype=torch.long), \
               str(self.df.iloc[idx]["input"]), str(self.df.iloc[idx]["target"])
...
def count_tags(df: pd.DataFrame) -> Tuple[int, int]:
    s = df["input"].astype(str)
    n_steps = int(s.str.startswith("<STEPS>").sum())
    n_ingr  = int(s.str.startswith("<INGR>").sum())
    return n_ingr, n_steps


def balanced_subset(df: pd.DataFrame, frac: float, seed: int) -> pd.DataFrame:
    """
    Sample the same fraction from INGREDIENT and STEPS rows (by input tag).
    If tags are missing, falls back to uniform sampling.
    """
    take = min(max(frac, 0.0), 1.0)
    if take >= 0.9999:
        return df.copy().reset_index(drop=True)

    tags = df["input"].astype(str).str.extract(r"^(<INGR>|<STEPS>)", expand=False)
    if tags.isna().all():
        # no tags; sample uniformly
        return df.sample(frac=take, random_state=seed).reset_index(drop=True)

    parts = []
    for tag, g in df.groupby(tags.fillna("<INGR>")):
        if len(g) == 0: continue
        parts.append(g.sample(frac=take, random_state=seed) if take < 1.0 else g)
...
def build_model_and_optim(sp: spm.SentencePieceProcessor) -> Tuple[nn.Module, AdamW, LambdaLR, int]:
    ...
    def inv_sqrt_lambda(step: int):
        ...
        if step <= cfg.WARMUP_STEPS:
            return step / cfg.WARMUP_STEPS  # linear warmup
...
def greedy_generate(model: nn.Module, sp: spm.SentencePieceProcessor, text: str, device, max_new_tokens: int = 128) -> str:
    ...
    for _ in range(max_new_tokens):
        ...
        # cast to bool to silence PyTorch deprecation warning (True=mask out future)
...
def train_one_epoch(model, loader, optimizer, scheduler, criterion, pad_id, scaler: Optional[GradScaler]) -> Tuple[float, float]:
    ...
    for step, (src, tgt, _, _) in enumerate(loader, 1):
        ...
        if cfg.LOG_EVERY_N_STEPS and (step % cfg.GRAD_ACCUM_STEPS == 0) and (step % cfg.LOG_EVERY_N_STEPS == 0):
            opt_step = step // cfg.GRAD_ACCUM_STEPS
            print(f"  [train] step {step}/{len(loader)} (opt {opt_step})  loss {loss.item():.4f}")
...
def fit():
    ...
    # Balanced subset BEFORE the split
    if cfg.SUBSET_FRAC < 1.0:
        n0 = len(full_df)
        full_df = balanced_subset(full_df, cfg.SUBSET_FRAC, cfg.SHUFFLE_SEED)
        n1 = len(full_df)
        n_ingr_all, n_steps_all = count_tags(full_df)
        print(f"Using balanced subset: {cfg.SUBSET_FRAC*100:.1f}% → {n1:,}/{n0:,} rows  "
              f"(INGR: {n_ingr_all:,} | STEPS: {n_steps_all:,})")
    ...
    # Show composition (for multitask sanity)
    n_ingr_v, n_steps_v = count_tags(val_ds.df)
    print(f"Val split rows -> INGREDIENTS: {n_ingr_v:,} | STEPS: {n_steps_v:,}")

    # Loaders (drop_last speeds training because shapes are consistent)
    num_workers = cfg.NUM_WORKERS
    prefetch_factor = cfg.PREFETCH_FACTOR if num_workers and num_workers > 0 else None
