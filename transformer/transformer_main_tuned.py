...
def train(model, train_loader, val_loader, optimizer, criterion, num_epochs, pad_idx):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        log_epoch(RESULTS_DIR, get_model_name_tuned(model), epoch,
                  train_loss, val_loss, val_ppl, val_acc, val_bleu, val_em, val_jacc, 0)

    # loss curve
    plt.plot(train_losses, label="Train")
    plt.plot(val_losses, label="Val")
    plt.xlabel("Epoch"); plt.ylabel("Loss"); plt.legend()
    plt.savefig(os.path.join(RESULTS_DIR, "loss_transformer_tuned.png"))
    plt.close()
    ...
    log_results(
        ...
        best_val_acc=best_val_acc, best_val_bleu=val_bleu, best_val_em=val_em,
...
if __name__ == "__main__":
    ...
    for lr, bs, dp in itertools.product(lrs, bss, dropouts):
        print(f"\n=== Run: lr={lr}, batch={bs}, dropout={dp} ===\n")
        ...
        model = Seq2SeqTransformerTuned(
            ...
            dim_ff=1024, dropout=dp, pad_idx=pad_idx
        ).to(DEVICE)

        criterion = nn.CrossEntropyLoss(ignore_index=pad_idx, label_smoothing=0.05)
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, betas=(0.9, 0.98), eps=1e-9, weight_decay=1e-5)

        best_loss, best_acc = train(model, train_loader, val_loader, optimizer, criterion, 20, pad_idx)

        results.append({
            "lr": lr, "batch": bs, "dropout": dp,
            "best_val_loss": best_loss,
            "best_val_acc": best_acc
        })

    df = pd.DataFrame(results)
    df.to_csv(os.path.join(RESULTS_DIR, "results_grid.csv"), index=False)
    print(df)
