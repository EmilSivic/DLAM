...
def train(model, train_loader, val_loader, optimizer, criterion, num_epochs, pad_idx, scheduler=None):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        log_epoch(RESULTS_DIR, get_model_name_tuned(model), epoch, train_loss, val_loss, val_ppl, val_acc, val_bleu, val_em, val_jacc, epoch_time)

    # loss curve
    plt.plot(train_losses, label="Train"); plt.plot(val_losses, label="Val")
    plt.xlabel("Epoch"); plt.ylabel("Loss"); plt.legend()
    plt.savefig(os.path.join(RESULTS_DIR, "loss_transformer_tuned.png")); plt.close()
...
if __name__=="__main__":
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=1.0,  # Noam Scheduler überschreibt
        betas=(0.9, 0.98),
        eps=1e-9,
        weight_decay=1e-5
    )
